...
class Dataloader(torch.utils.data.DataLoader):
    ...
    def preprocessing(self,dataframe):
        ...
        dataframe['persona'] = dataframe['persona'].apply(lambda x: ','.join(eval(x)).replace('.','')+'.') # ['A.','B.','C.'] -> 'A,B,C.'

        # tokenizing
        encoder_inputs = self.tokenizing(dataframe['session_dialog'], max_len=500)
        decoder_inputs = self.tokenizing(dataframe['persona'], max_len=200)
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            train_encoder_inputs, train_decoder_inputs= self.preprocessing(train_df)
            val_encoder_inputs, val_decoder_inputs  = self.preprocessing(val_df)

            self.train_dataset = Dataset(train_encoder_inputs, train_decoder_inputs)
            self.val_dataset = Dataset(val_encoder_inputs, val_decoder_inputs)
        else:
            test_df = pd.read_csv(self.test_path)
            predict_df = pd.read_csv(self.predict_path)

            test_encoder_inputs, test_decoder_inputs = self.preprocessing(test_df)
            predict_encoder_inputs, predict_decoder_inputs= self.preprocessing(predict_df)

            self.test_dataset = Dataset(test_encoder_inputs, test_decoder_inputs)
            self.predict_dataset = Dataset(predict_encoder_inputs, predict_decoder_inputs)
...
class Model(pl.LightningModule):
    ...
    def forward(self, **x):
        ...
        logits = self.model(input_ids=x['encoder_input_ids'], attention_mask=x['encoder_attention_mask'], 
                            labels=x['decoder_input_ids'], decoder_attention_mask=x['decoder_input_attention_mask']).logits
        
        return logits
    
    def training_step(self, batch, batch_idx):
        
        logits = self(**batch) # (batch_size, decoder_max_len, vocab_size)
        loss = self.loss(logits.view(-1, logits.shape[-1]), batch['decoder_input_ids'].view(-1)) # (batch_size*decoder_max_len, vocab_size), (batch_size*decoder_max_len)
...
if __name__ == '__main__':
    config = {"model_name": 'gogamza/kobart-base-v2',
              ...
              "dev_path":'./data/validation.csv',
              "test_path":'./data/validation.csv', 
              "predict_path":'./data/validation_csv',
              }
